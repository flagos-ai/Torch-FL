...
PY
  ...
  FLAGGEMS_GAP="$(gems_cohort_gap python || true)"
  if [[ -z "$FLAGGEMS_GAP" ]]; then
    echo "::error::The venv cannot import flag_gems at all; the 592 FlagGems" \
         "routes in backends_metax.conf have nothing to call."
    exit 1
  fi
