...
def _select_backend_config() -> None:
    ...
    suffix -- so the selection is a property of the build, not of an env var:
      ...
      * VENDOR_USE_BOXING=1          -> backends_metax.conf
    ...
    if os.environ.get("FLAGOS_BACKEND_CONFIG"):
        return
    vendor_boxing = os.environ.get("VENDOR_USE_BOXING", "0") == "1"
...
def _relink_vendor_libtorch() -> None:
    ...
    MetaX: VENDOR_USE_BOXING=1 triggers relink unconditionally (for in-place
...
def _patch_flaggems_codegen_config():
    ...
    - MetaX (boxing + FlagGems): set GEMS_VENDOR=metax so FlagGems uses the
      ...
      VENDOR_USE_BOXING=1 (or FLAGOS_METAX_COMPAT=1) and a MetaX card is present.
    ...
    if (
        ...
        and os.environ.get("VENDOR_USE_BOXING", "0") != "1"
