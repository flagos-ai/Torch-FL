...
# Regenerated by scripts/codegen_tileops.py. Used only to annotate
...
def main():
    ...
    if all_cuda:
        ...
        conf_lines = conf_license + [
            ...
            "# shim (scripts/backend_coverage.py:TILEOPS_OPS, regenerated by",
            "# scripts/codegen_tileops.py). It does not affect default routing --",
        ...
        # each other. The coverage set now lives in scripts/backend_coverage.py
        # as FLAGGEMS_PYTHON_OPS, and the per-op FlagGems routing is stated
        # directly in each platform's own conf by scripts/gen_vendor_confs.py.
