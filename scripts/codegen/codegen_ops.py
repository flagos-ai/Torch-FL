...
def main():
    ...
    if all_cuda:
        ...
        # in; these are the ones caused by FlagTree, which only the NVIDIA CUDA
        # pipeline runs, so they are deliberately NOT shared with the other
        # vendors.
        #
        # Current flag_gems master compiles addmm through _accumulate_dot, which
        ...
        conf_lines = conf_license + [
            ...
            "#",
            "# Exceptions: ops that pass the static discovery gates but are broken",
            "# at runtime under this platform's Triton are forced back to CUDA",
            "# boxing (scripts/codegen_ops.py:cuda_triton_fallback). The addmm",
            "# family is there because FlagTree AABS shrinks its tl.dot K tile",
