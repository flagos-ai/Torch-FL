...
# Regenerated by scripts/codegen/codegen_tileops.py. Used only to annotate
# backends_cuda.conf with `# tileops`; see the conf emission below for why the
# key is an annotation rather than a default route.
sys.path.insert(0, str(Path(__file__).resolve().parent))
from backend_coverage import TILEOPS_OPS  # noqa: E402
...
def discover_flaggems_ops(codegen_ops, funcs):
    ...
    try:
        ...
        # run codegen through scripts/vendor/with_cuda_libtorch.sh so torch_fl imports.
...
def main():
    ...
    if all_cuda:
        ...
        cuda_route_exceptions = {
            ...
            "_conj",
            # contract: flag_gems implements `_unsafe_view` as `self.reshape(size)`,
            # and reshape materialises a copy when the input is not viewable as the
            # requested shape. ATen's `_unsafe_view` instead returns a strided view
            # or raises "view size is not compatible with input tensor's size and
            # stride". Measured on A100: for a non-contiguous input the CUDA route
            # (and the CUDA boxing route through flagos) raises that RuntimeError
            # while the FlagGems route silently returns a materialised copy -- a
            # value-correct result with the wrong aliasing and twice the memory,
            # which no numerics comparison can see.
            "_unsafe_view",
        ...
        # Ops that carry no computation on either route: ATen implements them as
        # metadata (stride) manipulation, and flag_gems' implementation is a
        # Python function that calls back into the same thing, so no kernel is
        # gained by routing there -- only a Python hop per call.
        #
        # Measured on A100 at Qwen-Image-2.1's shapes, host cost per call:
        # unsqueeze 5.40 us on flaggems vs 2.09 us on CUDA boxing (1.86 us native
        # torch); _unsafe_view 7.07 vs 4.08 (3.70); transpose.int 5.14 vs 2.25
        # (1.98). The CUDA boxing route is within 0.3 us of native, so the win is
        # the Python hop and not the boxing.
        #
        # This is a cost exception, not a correctness one, and it is deliberately
        # modest: at 2.1's ~560 such calls per denoising step the change is worth
        # about 1.8 ms/step (0.35%), which is below this harness's run-to-run
        # noise -- an A/B of the whole 40-step pipeline could not resolve it. The
        # reason to take it is that the route costs something and gains nothing.
        ...
        conf_lines = conf_license + [
            ...
            "# stay lazy; `_unsafe_view` because flag_gems implements it as",
            "# `reshape`, which returns a copy where ATen returns a view or raises;",
            "# the addmm family because FlagTree AABS shrinks its tl.dot K tile",
            "# below Triton's K >= 16 minimum for small matmuls.",
            "#",
            ...
            "#",
            "# metadata_route_overrides holds a third, cost-only group: ops that do",
            "# no computation on either route, where flag_gems' implementation is a",
            "# Python function calling the same metadata operation back. Routing",
            "# them to flaggems buys no kernel -- there is none to buy -- and costs a",
            "# Python hop per call (measured: ~3 us/call more than the boxing route",
            "# on A100, which itself is within 0.3 us of native torch).",
            "#",
            ...
            "# scripts/codegen/codegen_tileops.py). It does not affect default routing --",
            "# TileOPs needs TILEOPS_KERNEL=ON plus an SM90 device, so naming it by",
            "# default would route at an empty slot. FLAGOS_USE_TILEOPS=1 repins"
            " exactly",
            "# these ops at load time (ApplyTileOpsOptIn in csrc/aten/common.cc).",
            "# The annotation replaces the former backends_tileops.conf, which was a",
            "# 2060-line copy of this file differing only in those 60 lines.",
            "#",
        ...
        n_flaggems_routes = 0
        n_cuda_fallback = 0
        n_metadata_override = 0
        for op in sorted(op_info):
            if op in metadata_route_overrides:
                backend = "cuda"
                if op in flaggems_py:
                    n_metadata_override += 1
            elif op in cuda_route_exceptions or op in measured_flaggems_rollback:
                backend = "cuda"
                if op in flaggems_py:
                    n_cuda_fallback += 1
            else:
        ...
        if n_metadata_override:
            print(
                f"   {n_metadata_override} metadata-only ops kept off the FlagGems "
                f"route (metadata_route_overrides)"
            )
        ...
        #
        # There is no longer a generic backends_flaggems.conf to write here. It
        # was one file per *mode* rather than per platform, and it doubled as the
        # generator's FlagGems coverage source, so its two jobs pulled against
        # each other. The coverage set now lives in scripts/codegen/backend_coverage.py
