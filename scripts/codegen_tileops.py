...
``torch_fl/tileops_spec.py`` and writes:
  ...
  - ``torch_fl/generated/tileops_shims.py``       module-level callables for C++
...
``torch_fl.generated.tileops_shims`` via ``CallPythonOp_Generic``.
...
OUT_SHIMS = REPO / "torch_fl" / "generated" / "tileops_shims.py"
...
SHIM_MODULE = "torch_fl.generated.tileops_shims"
...
#: aten params the recipe builders in torch_fl/tileops_backend.py read from
...
def render_shims(routes: List[Route], sigs: Dict[str, CppSig]) -> str:
    """Emit torch_fl/generated/tileops_shims.py.
    ...
    ``torch_fl.tileops_backend`` -- these shims are just the named entry points.
...
def render_conf(routes: List[Route]) -> str:
    ...
    head = [
        ...
        "#",
        "# These entries drive the C++ dispatcher: TileOPs kernels register on",
        "# Backend::kTileOps (csrc/aten/generated/tileops_python_kernels.cc), so an",
        "# op routes here only if this file says so. Revert one with",
        "# FLAGOS_OP_<name>=cuda; trace routing with FLAGOS_LOG_DISPATCH=1.",
        "#",
...
def main() -> int:
    ...
    if unknown:
        ...
        sys.exit(
            ...
            + ", ".join(sorted(unknown))
            + "\nre-run scripts/codegen_ops.py, or exclude these in tileops_spec.EXCLUDE"
        )
