...
```
tileops/manifest/*.yaml ──┐                                ┌─► torch_fl/tileops/generated/routes.py
                          │                                ├─► torch_fl/tileops/generated/shims.py
                          ├─► scripts/codegen/codegen_tileops.py ───┼─► csrc/aten/generated/tileops_python_kernels.cc
                          │      (run offline,              ├─► torch_fl/configs/backends_tileops.conf
torch_fl/tileops/spec.py ─┘       output committed)         └─► tests/integration/ops/test_tileops_generated.py
  (recipe + alias + exclusion
   tables, hand-maintained)
          ...
          └─► ReluKernelTileOps             csrc/aten/generated/tileops_python_kernels.cc
              └─► CallPythonOp_Generic      csrc/aten/backends/flagos/python_op_caller.cc
                  └─► _shim_relu            torch_fl/tileops/generated/shims.py
                      └─► resolve_impl      torch_fl/tileops/runtime.py
...
```
torch_fl/tileops/
    __init__.py       docs the split; imports nothing eagerly
    runtime.py        instance cache, boxing, the four recipe builders
    spec.py           hand-maintained codegen input (aliases, recipes, exclusions)
    generated/
        routes.py     the routing table (ROUTES, WORKLOADS)
        shims.py      one free function per route, resolved by name from C++
```

`runtime.py` was called `tileops_backend.py` while registration lived in Python.
It is not the backend any more -- `tileops_python_kernels.cc` is -- so the name
now says what it is: the runtime the generated shims land in. Note that
`shims.py`'s module path is load-bearing in a way the others are not: the C++
stubs embed it as a string (`SHIM_MODULE` in the generator), so moving it
without regenerating breaks each route at first call rather than at import.

Three design principles:

1. **The manifest is the only spec source.** `tileops/spec.py` holds only what
...
### 3.1 Input: `torch_fl/tileops/spec.py` (hand-maintained)
...
### 3.2 Generator: `scripts/codegen/codegen_tileops.py`

The flow mirrors how `scripts/codegen/codegen_ops.py` is organized (read spec ->
...
### 3.3 Output: `torch_fl/tileops/generated/routes.py`

Recipes render as table-driven entries rather than one function body per
operator:

```python
# AUTO-GENERATED by scripts/codegen/codegen_tileops.py -- DO NOT EDIT
...
`Cumsum`/`Cumprod` (reasons in `tileops.spec.EXCLUDE`).
...
**`torch_fl/tileops/generated/shims.py`** -- one module-level free function per
...
Disposition (implemented in `tileops.runtime._disable_frontend_cache()`):
...
excluded 12 (each with a measured reason, see tileops.spec.EXCLUDE)
