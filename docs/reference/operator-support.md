...
(byte-identical to the base commit's copy, **3 passed in 8.02s** alone, the same
...
than against the 467-route one this change ships. The entry's evidence is a CI
failure and the CI re-run that follows it, not a local device run — there is no
MUSA host in this session. The generated kernel has **no local compile check**,
since the MUDNN toolchain is not installed here. No timing was taken, so nothing
is claimed about whether mudnn's `SELECT` is faster than FlagGems'
`where_self_out` at the shapes this overload is called with; the change is
justified by the contract alone. And the same contract gap remains on `cuda`,
`dcu`, `metax` and `ppu`, which still route the overload to FlagGems and whose
manifests do not collect the case — the two candidate remedies (the same
