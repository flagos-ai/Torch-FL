...
see, which are pinned in the same place, so the shipped configuration is **439
`flaggems` / 1597 `cuda`**: 43 of the 482 FlagGems-covered ops are held on the
vendor kernel — the 33 route-dependent failures measured below, the mm/bmm
family, five `addmm` overloads and `_conj` — and the other 439 route to FlagGems.
The shipped file is idempotent (SHA-256
`cc989837da9d3e923e8fc8ca79e005420bbb717457145d33064ce1fc659e5658`, reproduced
byte-for-byte by a second `gen_vendor_confs.py` run).
...
**The route exceptions.** 43 FlagGems-covered ops stay on the `cuda` boxing
...
**The six exceptions the survey cannot see.** `addmm`, `addmm.dtype`,
`addmm.dtype_out`, `addmm.out`, `addmm_` and `_conj` were pinned after the first
CI run on the 445-route configuration, because both failures are invisible to a
value-level overload survey and only the integration suites reach them.
...
439-route table hashed above, and the two hashes therefore differ by design. The
six ops pinned after that run are by definition outside its scope — the survey
never measured them, and the paragraph above records what did.
