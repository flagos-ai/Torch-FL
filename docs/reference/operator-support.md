...
both accessors of both orientations, `values`, `empty`, both conversions,
`to_dense`, `spmm`, `clone` and the device round-trip -- each with

```text
RuntimeError: false INTERNAL ASSERT FAILED at "...DispatchStub.cpp":275, ...
DispatchStub: missing kernel for flagos
```

for the two conversions and `Could not run 'aten::empty.memory_format' with
arguments from the 'SparseCsrflagos' backend` for the allocation, i.e. the whole
surface fell through to defaults that do not exist for this key. After the change
the two conversions on their own report:
...
`tests/integration/ops/test_sparse_csr_dispatch.py` pins the surface: 32 checks
...
as every case's error. That inconsistency is a separate defect, is not addressed
here, and the shim is not part of this change. The eight fp16 failures
