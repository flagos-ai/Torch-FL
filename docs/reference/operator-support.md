...
| 2026-08-31 | Kunlun P800 XPU | Kunlun AMP GradScaler route (`_amp_foreach_non_finite_check_and_unscale_`) | Preserved `found_inf` across successive `(device, dtype)` gradient-group calls so an overflow in one dtype group cannot be cleared by a later finite group. Added mixed-bfloat16/float32 regression coverage; FlagGems and generic 546-route cohorts are **not revalidated** because their active routes are unchanged. | Measured on P800 OAM (XPU-RT 5.37.1, torch 2.9.0+cu129, `ACCELERATOR=kunlun`, `FLAGOS_USE_FLAGGEMS=0`, device `flagos:0`, 8 devices). `tests/integration/test_amp.py`: `18 passed in 0.24s`, including direct found-inf accumulation and an end-to-end overflow step with bfloat16 and float32 gradient groups; the latter left both parameters unchanged and backed the scale off from 8.0 to 4.0. Distributed AMP (DDP/FSDP2 with GradScaler) and AMP performance remain **not validated** on this hardware. |
