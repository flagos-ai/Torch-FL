...
The 5060 row is a separate cohort: it measures the current 538-route
configuration (seven device-assert ops -- `i0`, `i0.out`, `special_i0e`,
`special_i0e.out`, `special_i1`, `upsample_bicubic2d`, `soft_margin_loss` --
rerouted to CUDA boxing on 2026-08-24), so its 538 denominator is not directly
comparable with the 546-route rows above. The four 546-route rows are
**not revalidated** against the new cohort.
